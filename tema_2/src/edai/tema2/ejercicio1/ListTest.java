...
class ListTest {
	...
	void testInsertTwoAtEnd() {
		list.insert("Hello", -1).insert("Last", -1);
		assertEquals("Hello", list.getFirst().getData());
		assertEquals("Last", list.getFirst().getNext().getData());
		assertEquals(2, list.size());
		assertFalse(list.isEmpty());
		assertArrayEquals(new String[] { "Hello", "Last" }, list.listData());
	}

	@Test
	void testInsertAtPosition() {
		list.insert("First", 0).insert("Second", 1).insert("Third", 2);
		assertEquals("First", list.getFirst().getData());
		assertEquals("Second", list.getFirst().getNext().getData());
		assertEquals("Third", list.getFirst().getNext().getNext().getData());
		assertEquals(3, list.size());
		assertFalse(list.isEmpty());
		assertArrayEquals(new String[] { "First", "Second", "Third" }, list.listData());
	}

	@Test
	void testInsertInMiddle() {
		list.insert("First", 0).insert("Second", 1).insert("Third", 2).insert("Middle", 2);
		assertEquals("First", list.getFirst().getData());
		assertEquals("Second", list.getFirst().getNext().getData());
	...
	void testInsertAtIndexBelowBounds() {
		assertThrows(IndexOutOfBoundsException.class, () -> list.insert("Invalid", -2));
	}
	...
	void testRemoveAtPosition() {
		list.insert("First", -1).insert("Second", -1).insert("Third", -1);
		list.remove(1);
		assertEquals(2, list.size());
		assertFalse(list.isEmpty());
	...
	@Test
	void testRemoveAboveBoundsInEmpty() {
